...
class Employees extends Vtiger_CRMEntity
{
	 ...
	 */
	public $customFieldTable = ['vtiger_employeescf', 'employeesid'];

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	public $tab_name = ['vtiger_crmentity', 'vtiger_employees', 'vtiger_employeescf', 'vtiger_entity_stats'];
	...
	public $list_fields = [
		...
		'LBL_LASTNAME' => ['employees', 'last_name'],
		'LBL_NAME' => ['employees', 'name'],
		'LBL_BUSINESSPHONE' => ['employees', 'business_phone'],
		'LBL_BUSINESSMAIL' => ['employees', 'business_mail'],
	...
	public function getEmployeeHierarchy($id)
	{
		...
		foreach ($this->list_fields_name as $fieldName => $colName) {
			if (\App\Field::getFieldPermission('Employees', $colName)) {
		...
		foreach ($rowsList as $employeesId => $accountInfo) {
			$accountInfoData = [];
			$hasRecordViewAccess = \App\Privilege::isPermitted('Employees', 'DetailView', $employeesId);
			foreach ($this->list_fields_name as $fieldName => $colName) {
				if (!$hasRecordViewAccess && $colName != 'name') {
					$accountInfoData[] = '';
				} elseif (\App\Field::getFieldPermission('Employees', $colName)) {
					$data = \App\Purifier::encodeHtml($accountInfo[$colName]);
					if ($colName == 'name') {
						if ($employeesId != $id) {
							if ($hasRecordViewAccess) {
								$data = '<a href="index.php?module=Employees&view=Detail&record=' . $employeesId . '">' . $data . '</a>';
							} else {
								$data = '<i>' . $data . '</i>';
							}
						} else {
	...
	public function __getParentEmployees($id, &$parentAccounts, &$encounteredAccounts)
	{
		...
		$parentId = (new App\Db\Query())
			...
			->innerJoin('vtiger_crmentity', 'vtiger_employees.id = vtiger_crmentity.crmid')
			->where(['vtiger_crmentity.deleted' => 0, 'vtiger_employees.employeesid' => $id])->scalar();
		...
		$data = (new App\Db\Query())
			->select(['vtiger_employees.*', 'user_name' => new \yii\db\Expression('CASE when (vtiger_users.user_name not like ' . App\Db::getInstance()->quoteValue('') . ") THEN $userNameSql ELSE vtiger_groups.groupname END")])
			->from('vtiger_employees')
			->innerJoin('vtiger_crmentity', 'vtiger_employees.employeesid = vtiger_crmentity.crmid')
			...
			->where(['vtiger_crmentity.deleted' => 0, 'vtiger_employees.employeesid' => $id])
			->one();
		$parentAccountInfo = [];
		$depth = 0;
	...
	public function moduleHandler($moduleName, $eventType)
	{
		if ($eventType == 'module.postinstall') {
			...
			\App\Db::getInstance()->createCommand()->update('vtiger_field', ['summaryfield' => 1], ['and', ['tabid' => $tabId],
				['columnname' => ['employees_no', 'employee_status', 'name', 'last_name', 'pesel', 'id_card', 'employee_education', 'parentid', 'business_mail']], ])->execute();
			// block with comments
